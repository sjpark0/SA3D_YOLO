...
class Sam3D(ABC):
    ...
    def __init__(self, args, cfg, xyz_min, xyz_max, cfg_model, cfg_train, \
        ...
        self.view_id_to_index = {view_id: index for index, view_id in enumerate(self.view_ids)}
        self.index_to_view_id = {index: view_id for index, view_id in enumerate(self.view_ids)}
    ...
    def _sort_training_data(self):
        ...
        for idx in self.data_dict['i_train']:
            ...
            if len(results[0].boxes) > 0:
                confidences.append((idx, results[0].boxes.conf.max().item()))
            else:
                confidences.append((idx, 0.0))
