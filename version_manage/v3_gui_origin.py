...
def calculate_pose_distances(poses):
    ...
    # Start from the first pose and add each closest pose to sorted_indices
    sorted_indices = [0]  # Start with the first pose
    remaining_indices = set(range(1, num_poses))

    while remaining_indices:
        last_pose = sorted_indices[-1]
        # Get the closest pose to the last added pose
        closest_pose = min(remaining_indices, key=lambda idx: distance_matrix[last_pose, idx])
        sorted_indices.append(closest_pose)
        remaining_indices.remove(closest_pose)

    logging.info(f"Original order: {list(range(num_poses))}")
    logging.info(f"New pose order: {sorted_indices}")
    ...
    def run_app(self, sam_pred, ctx, init_rgb):
        ...
        def start_training(btn):
            if btn < 1:
                ...
                distances = calculate_pose_distances(poses)
                logging.debug(f"Calculated {len(distances)} distances between poses")

                # Sort views based on pose distances
                sorted_indices = []
                current_index = 0
                while len(sorted_indices) < len(poses):
                    sorted_indices.append(current_index)
                    logging.debug(f"Added index {current_index} to sorted_indices")
                    if len(sorted_indices) == len(poses):
                        break
                    next_distances = [d for d in distances if d[0] == current_index and d[1] not in sorted_indices]
                    if not next_distances:
                        next_distances = [d for d in distances if d[1] == current_index and d[0] not in sorted_indices]
                    if not next_distances:
                        remaining = set(range(len(poses))) - set(sorted_indices)
                        current_index = min(remaining)
                    else:
                        _, current_index, _ = min(next_distances, key=lambda x: x[2])
                        logging.debug(f"Selected next index: {current_index}")

                
                i_train_sorted = [self.Seg3d.data_dict['i_train'][i] for i in sorted_indices]

                self.Seg3d.data_dict['i_train'] = i_train_sorted

                logging.debug(f"Original i_train: {self.Seg3d.data_dict['i_train']}")
                logging.debug(f"Sorted i_train: {i_train_sorted}")
