    ...
    def run_app(self, sam_pred, ctx, init_rgb):
        ...
        def start_training(btn):
            if btn < 1:
                ...
                # 학습 순서 변경 플래그 설정
                self.Seg3d.vsgflag = True  # 할당 연산자 수정
                ...
                # max_idx = confidence.index(max(confidence)

                # i_train_sorted = list(view_dict.values())

                # 이런식으로 해보기, 위의 저 플래그도 살려서 꼭 해야함
                ...
                sorted_indices = calculate_pose_distances(poses)  # 정렬된 포즈 인덱스
                ...
                logging.info(f"Original order: {view_ids}")
                logging.info(f"New pose order: {sorted_view_ids}")
                logging.info(f"Sorted training order: {i_train_sorted}")
                ...
                #     m = tf(PIL.Image.open(f'../Labeling/Set1/masks/{num_list[i_train_sorted[idx]]:02d}_p1.png'))
